	...
	protected void service(HttpServletRequest req, HttpServletResponse res)
		...
		if (cat == null) {
			...
			try {
				...
				ps = con.prepareStatement("select customer_id,cust_name from customer1 where email_id=?");
		...
		if (cat.equals("add")) {
			...
			if (otp.equals("1234")) {
				...
				try {
					...
					if (rs.next()) {
						...
						pw.println("<HTML>");
						pw.println("<head>");
						pw.println("<link rel=\"stylesheet\" type=\"text/css\" href=\"common.css\">");
						pw.println("</head>");
						pw.println("<body>");
				...
				pw.println("<form action=\"login.jsp\">");
				pw.println("<input type=\"submit\" value=\"Click here to try again.\">");
				pw.println("</body>");
				pw.println("</HTML>");
			}
			...
			try {
				...
				PreparedStatement ps = con
						.prepareStatement("select product_id from mycart where  CUSTOMER_ID=?");
				ps.setString(1, cid);

				ResultSet rs = ps.executeQuery();
				while(rs.next()){
				quant=req.getParameter(rs.getString(1));
				OrderBean ob = new OrderBean(rs.getString(1), mode, cid, quant);
				result = os.pay(ob);
				}
			} catch (Exception e) {
				System.out.println(e);
